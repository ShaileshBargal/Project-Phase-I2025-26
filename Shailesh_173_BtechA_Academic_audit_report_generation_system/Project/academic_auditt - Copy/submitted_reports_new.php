...
if (!isset($_SESSION['head_id']) || ($_SESSION['role'] ?? '') !== 'head') {
    exit("Unauthorized - Please login as Head");
}

// Detect if loaded via AJAX (from head.php)
  ...
  <div class="tab-navigation" style="align-items:center;">
    ...
    <div style="margin-left:auto;display:flex;align-items:center;gap:10px">
      ...
      <select id="yearFilter" style="padding:8px 12px;border:2px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;cursor:pointer;" onchange="changeYear()">
        <?php
        $current_year = date('Y');
        // Generate years: current-1 to current+3
        $years = range($current_year - 1, $current_year + 3);
        foreach($years as $y){
            $selected = ($y == 2024) ? 'selected' : ''; // Default to 2024 as per requirements or make it dynamic
            // Better: default to current year if not known
...
<script>
(function(){
    // Use window scope for variables accessed by multiple function cals
    // But protect against redeclaration if possible, though assigning to window property is safe
    
    // Define functions on window so onclick handlers work
    window.currentSemester = window.currentSemester || 'odd';
        ...
        if (value === 'add_new') {
            ...
            if (newYear && /^\d{4}$/.test(newYear)) {
                ...
                if(!exists){
                    ...
                    // Insert before the last option (Add Year)
                    let lastOption = select.options[select.options.length - 1];
                    select.insertBefore(option, lastOption);
                    select.value = newYear;
                }
                window.currentYear = newYear;
                loadReports(window.currentSemester);
            } else {
                // Invalid or cancelled, revert
                if(newYear !== null) alert("Please enter a valid 4-digit year.");
                select.value = window.currentYear;
            }
        } else {
            window.currentYear = value;
            loadReports(window.currentSemester);
        }
    }

    window.switchTab = function(semester){
    ...
    window.loadReports = function(semester){
        ...
        console.log('Loading reports for semester:', semester, 'year:', year);
        
        $.get('get_submitted_reports.php', {semester, year: year}, function(resp){
            ...
            if(resp.ok && resp.reports.length > 0){
                ...
                resp.reports.forEach(r => {
                    ...
                    <tr>
                        ...
                        <td>${r.reviewed_at || '-'}</td>
<td><a href="${r.file_url}" target="_blank" class="download-btn">View</a></td>
                        <td>${actionCol}</td>
                    </tr>
                    `;
                });
